...
function query2sql(query: z.infer<typeof MetricsQuerySchema>, event: H3Event): string {
  ...
  const sql = select(`${logsMap[query.type]} as name, blob16 as app, SUM(_sample_interval) as count`).from(dataset).where(filter).groupBy('name','app').orderBy('count DESC').limit(query.limit)
  if (query.type === 'slug'){
